...
![Scaling overview](figures/scaling_overview.png)

*Generated by `plot_scaling.py` — the same data shown as line charts to highlight
the throughput trend as thread count increases. Each panel covers one type size
(row) and one workload (column): LockFree (solid blue) vs Mutex (dashed red).*
